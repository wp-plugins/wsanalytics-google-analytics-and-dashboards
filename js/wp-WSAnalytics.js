...
jQuery(document).ready(function ($) {
	...
	$('#populate_credss').on("click",function(){
		...
		$.ajax({
				...
				success: function (data, textStatus, XMLHttpRequest) {
					...
					$('#WSAnalytics_clientid').val(obj[0].id);
					$('#WSAnalytics_clientsecret').val(obj[0].secret);
					$('#WSAnalytics_apikey').val(obj[0].key);
				},
				error: function (MLHttpRequest, textStatus, errorThrown) {
					alert("We Couldn't fetch the keys, Please create your own from Google console.");
				}
		});
	});
