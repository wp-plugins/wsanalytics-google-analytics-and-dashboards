...
class Google_ServiceResource {
  ...
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new WSAnalytics_Google_Exception("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    if ($postBody) {
      ...
      if (isset($contentType) && $contentType) {
        ...
        $contentTypeHeader['content-length'] = WSAnalytics_Google_Utils::getStrLen($postBody);
      }
      $httpRequest->setRequestHeaders($contentTypeHeader);
    }

    $httpRequest = WSAnalytics_Google_Client::$auth->sign($httpRequest);
    if (WSAnalytics_Google_Client::$useBatch) {
      return $httpRequest;
    }
